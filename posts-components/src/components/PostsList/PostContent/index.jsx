...
const PostContent = ({ usersPhotos, usersList, idx, post }) => {
  ...
  const postDate = new Date(time).toLocaleString('en-US', { hour12: false, hour: 'numeric', minute: 'numeric', year: 'numeric', month: 'short', day: 'numeric' });
    ...
    <div className="post__content">
      ...
      <div className="post__title">
        <Link className="post__link" key={post.id} to={`/post/${post.id}`}>
          {idx + 1}. {post.title[0].toUpperCase() + post.title.slice(1)}<br />
